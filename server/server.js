...
app.post('/api/download/theme', async (req, res) => {
  try {
    ...
    // Add theme.txt
    const themeContent = generateThemeTxt(config);
    archive.append(themeContent, { name: 'theme.txt' });

    // Add ventoy.json
    const ventoyJson = generateVentoyJson(config);
    archive.append(JSON.stringify(ventoyJson, null, 2), { name: 'ventoy.json' });
    ...
    Object.entries(config.iconFiles || {}).forEach(([iconType, filename]) => {
      if (filename && fs.existsSync(path.join(iconsDir, filename))) {
        ...
        archive.file(path.join(iconsDir, filename), { name: `icons/${iconType}${ext}` });
      }
    });
...
function generateThemeTxt(config) {
  ...
  return `# ============================================
# Ventoy Pro Theme Configuration
...
| \`ventoy.json\` | Ventoy settings & mappings |
| \`README.md\` | This file |
| \`INSTALL.md\` | Installation guide |
| \`backgrounds/\` | Background images |
| \`icons/\` | OS-specific icons |
...
### Step 2: Copy to USB
Copy ALL files to your Ventoy USB drive's \`ventoy\` folder:
